   ...
   return (
      <motion.section key={location.key}>
         ...
         <motion.div key={slug} className="grid grid-cols-1 gap-10 lg:gap-4" {...framerAnimations.slideRightFadeIn}>
            <div className="md:py-4 xl:py-16">
            ...
            <div className="w-full px-4 xl:px-10 flex justify-between mb-6 md:mb-10 xl:mb-16 xl:mt-8">               
               <Link to={`/portfolio/${previousEntry.slug}`} className="text-center md:text-lg xl:text-xl">
                  <i className="fa-solid fa-arrow-left"></i> {previousEntry.title}
               </Link>
               <Link to={`/portfolio/${nextEntry.slug}`} className="text-center md:text-lg xl:text-xl">
